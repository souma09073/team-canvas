...
class Screens {
  ...
  void drawReady() {
    ...
    fill(255, 230);
    text("←→ / A・D:レーン移動　スペース:マンジャロ　Z:エナジードリンク　R:リトライ", SCREEN_W * 0.5, 280);
    ...
    // ゾーンは廃止し、エナジードリンクに置き換えた。
    // 「代償がある」ことを先に伝えないと、使って倒れた理由が分からない。
    text("エナジードリンクは各エリアに1本。Zキーで " + int(ENERGY_DURATION) + "秒間 加速できるが、"
       + "糖分のかたまりなので血糖が +" + int(ENERGY_GAIN) + " 跳ね上がる。", SCREEN_W * 0.5, 392);
